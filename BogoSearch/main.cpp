...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                if (event.size.width < drawable_calculations::initial_win_size.x)
                {
                    window.setSize(sf::Vector2u(drawable_calculations::initial_win_size.x, window.getSize().y));            
                }
                if (event.size.height < drawable_calculations::initial_win_size.y)
                {
                    window.setSize(sf::Vector2u(window.getSize().x, drawable_calculations::initial_win_size.y));
                }
        ...
        renderTexture.clear();
        st.state_manager_rt(window, renderTexture, main_view);
